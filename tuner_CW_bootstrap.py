...
directory = "LSTM_tuning_logs_CW_6/LSTM_hypo_6"
os.makedirs(directory, exist_ok=True)

print("CW Tuner - Curated")
...
def train_generator(batch_size):
    ...
    for i, (mchunk, lchunk, dchunk, mealchunk, schunk, ychunk) in enumerate(zip(
        ...
        # Create sample weights using class weights
        sample_weights = np.where(labels == 1, class_weight_dict[1.0], class_weight_dict[0.0]).flatten()

        yield inputs, labels, sample_weights
        ...
        output_signature=(
            {
                ...
                "TIMESERIES_INPUT_meal": tf.TensorSpec(shape=(None, n_timesteps_meal, 1), dtype=tf.float32),
...
def build_model(hp):
    ...
    # Predefine all possible LSTM units and dropout rates for 3 layers
    #for i in range(1, 4):
        #hp.Int(f'lstm_units_layer_{i}', min_value=200, max_value=256, step=8)
        #hp.Float(f'dropout_rate_layer_{i}', min_value=0.8, max_value=0.9, step=0.05)
    for i in range(1, 4):
        if i == 1:
    ...
    recurrent_input_meal = tf.keras.Input(shape=(n_timesteps_meal, 1), name="TIMESERIES_INPUT_meal")
    static_input = tf.keras.Input(shape=(65,), name="STATIC_INPUT")
...
tuner = BayesianOptimization(
    ...
    directory='LSTM_tuning_logs_CW_6',
...
# Load datasets
train_dataset = dataset_from_generator(train_generator, batch_size).repeat().take(steps_per_epoch * epochs).prefetch(tf.data.AUTOTUNE)
val_dataset = dataset_from_generator(val_generator, batch_size).repeat().take(validation_steps * epochs).prefetch(tf.data.AUTOTUNE)
...
pd.DataFrame({"y_pred_prob": y_pred_probs.flatten()}).to_csv("CW_y_pred_probs.csv", index=False)
...
bootstrap_df.to_csv("CW_bootstrap_metrics.csv", index=False)
...
plt.legend()
plt.savefig('CW_train_val_loss.svg', bbox_inches='tight')